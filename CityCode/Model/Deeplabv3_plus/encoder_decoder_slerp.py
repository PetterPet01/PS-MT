...
class DecoderNetwork(nn.Module):
    ...
    def forward_features(self, f):
        """ New method to return only the deep features before classification. """
        # The Head module in the encoder already does the concat and first convs
        # The upsample module takes it from there
        features_from_head = self.upsample.last_conv(f)
        return features_from_head

    def forward(self, f, data_shape=None):
        """ The original forward pass now produces logits. """
        features = self.forward_features(f)
        pred = self.upsample.classifier(features)

        if self.training:
            h, w = self.upsample.data_shape[0], self.upsample.data_shape[1]
        else:
            if data_shape is not None:
                h, w = data_shape[0], data_shape[1]
            else:
...
class VATDecoderNetwork(nn.Module):
    ...
    def forward_features(self, f, t_model=None):
        """ New method to return student's deep features, with VAT. """
        if t_model is not None:
            r_adv = get_r_adv_t(f, t_model[0], t_model[1], it=1, xi=1e-6, eps=2.0)
            f += r_adv

        return self.upsample.last_conv(f)

    def forward(self, f, data_shape=None, t_model=None):
        """ The original forward pass now produces logits. """
        features = self.forward_features(f, t_model=t_model)
        pred = self.upsample.classifier(features)
