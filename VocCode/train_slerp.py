...
class Trainer(BaseTrainer):
    ...
    @torch.no_grad()
    def get_teacher_features_with_out_grad(self, image):
        """ Gets the deep features from the teacher models. """
        with torch.no_grad():
            # Get raw encoder features first
            teacher1_enc_feat = self.model.module.encoder1(image)
            teacher2_enc_feat = self.model.module.encoder2(image)
            
            # Now pass through the feature part of the decoder
            features1 = self.model.module.decoder1.forward_features(teacher1_enc_feat)
            features2 = self.model.module.decoder2.forward_features(teacher2_enc_feat)
            
            assert features1.shape == features2.shape, "Expect two feature maps in same shape"
    ...
    def _train_epoch(self, epoch, id):
        ...
        for batch_idx in tbar:
            ...
            un_loss_type = self.model.module.un_loss_type

            if self.mode == "semi":
                if un_loss_type == 'feature_slerp':
                    # --- NEW WAY ---
                    t1_features, t2_features = self.get_teacher_features_with_out_grad(input_ul_wk)
                    
                    from Utils.losses import slerp # Make sure this is imported
                    predict_target_ul = slerp(t1_features, t2_features, alpha=(1 - self.gamma))
                    predict_target_ul = predict_target_ul.detach()
                
                else: # 'conf_ce' - OLD WAY
                    t1_prob, t2_prob = self.predict_with_out_grad(input_ul_wk)
                    if id == 1:
                        t2_prob = self.assist_mask_calculate(...)
                    else:
                        t1_prob = self.assist_mask_calculate(...)
