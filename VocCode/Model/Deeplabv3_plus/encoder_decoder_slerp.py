...
class VATDecoderNetwork(nn.Module):
    ...
    def forward_features(self, f, t_model_features=None):
        if t_model_features is not None:
            teacher1_full_decoder, teacher2_full_decoder = t_model_features
            r_adv = get_r_adv_t(f, teacher1_full_decoder, teacher2_full_decoder, it=1, xi=1e-6, eps=2.0)
            f += r_adv
        
        return self.upsample.last_conv(f)

    def forward(self, f, data_shape=None, t_model=None):
        features = self.forward_features(f, t_model_features=t_model)
        pred = self.upsample.classifier(features)
